...
int do_fork( process* parent)
{
  ...
  for( int i=0; i<parent->total_mapped_region; i++ ){
    ...
    switch( parent->mapped_info[i].seg_type ){
      ...
      case DATA_SEGMENT: {
        void *pa = alloc_page();
        memcpy(pa, (void *)lookup_pa(parent->pagetable, parent->mapped_info[i].va), PGSIZE);
        user_vm_map(child->pagetable, parent->mapped_info[i].va, PGSIZE, (uint64)pa, prot_to_type(PROT_READ | PROT_WRITE | PROT_EXEC, 1));
