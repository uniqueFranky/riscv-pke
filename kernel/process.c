    ...
    switch( parent->mapped_info[i].seg_type ){
      ...
      case HEAP_SEGMENT: {
        ...
        for(int i = 0; i < PROC_MAX_PAGE_NUM; i++) {
          if(parent->page_cb[i].valid) {
            void *pa = alloc_page();
            child->page_cb[i] = parent->page_cb[i];
            child->page_cb[i].start_pa = (uint64)pa;
            user_vm_map(child->pagetable, child->page_cb[i].start_va, PGSIZE, (uint64)pa, prot_to_type(PROT_READ | PROT_WRITE, 1));
            memcpy(pa, (void *)parent->page_cb[i].start_pa, PGSIZE);
          }
        }

        child->mapped_info[HEAP_SEGMENT].npages = parent->mapped_info[HEAP_SEGMENT].npages;

        memory_descriptor *now = parent->free_md_list_head;
        while(NULL != now) {
          memory_descriptor *md = alloc_memory_descriptor();
          copy_md_from_parent(now, md, parent, child, 0);
          now = now->next;
        }
        now = parent->allocated_md_list_head;
        while(NULL != now) {
          memory_descriptor *md = alloc_memory_descriptor();
          copy_md_from_parent(now, md, parent, child, 0);
          now = now->next;
      ...
      case DATA_SEGMENT: {
        for(int j = 0; j < parent->mapped_info[i].npages; j++) {
          ...
          user_vm_map(child->pagetable, parent->mapped_info[i].va + j * PGSIZE, PGSIZE, (uint64)pa, prot_to_type(PROT_READ | PROT_WRITE | PROT_EXEC, 1));
        }
        child->mapped_info[child->total_mapped_region].va = parent->mapped_info[i].va;
